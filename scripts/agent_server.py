...
def _select_data_plane(mode: str, *, tcp_binary: Path | None = None):
    if mode.upper() == "RDMA":
        return RDMADataplane()
    return TCPDataPlane(binary_path=tcp_binary)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional path to the compiled dms_tcp_transfer binary",
    )
    ...
    data_plane = _select_data_plane(args.mode, tcp_binary=args.tcp_binary)
