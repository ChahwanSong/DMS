...
def test_scheduler_policy_override(tmp_path: Path) -> None:
    source_root = tmp_path / "source"
    dest_root = tmp_path / "dest"
    source_root.mkdir()
    dest_root.mkdir()
    ...
    plans = scheduler.plan(request)
    assert plans["agent-1"].total_bytes == sum(len(v) for v in files.values())
    assert plans["agent-2"].total_bytes == 0
    assert len(plans["agent-1"].assignments) == len(files)
    assert all(assignment.chunk.offset == 0 for assignment in plans["agent-1"].assignments)
